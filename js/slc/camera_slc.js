var bcc_cam_links = ['https://udottraffic.utah.gov/1_devices/aux14605.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16212.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16213.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16215.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16216.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux18040.jpeg',
    'http://udottraffic.utah.gov/1_devices/SR-190%20MP%2015%2095%20SL.gif',
    'http://www.udottraffic.utah.gov/AnimatedGifs/100033.gif']

var lcc_cam_links = ['https://udottraffic.utah.gov/1_devices/aux16607.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux14604.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16265.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16267.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16269.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux16270.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux17227.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux17228.jpeg',
    'https://udottraffic.utah.gov/1_devices/aux17226.jpeg',
    'http://www.udottraffic.utah.gov/AnimatedGifs/100032.gif']

var i80_cam_links = ['http://udottraffic.utah.gov/1_devices/aux166.jpeg']

document.getElementById('bcc_cam0').src = bcc_cam_links[0];
document.getElementById('bcc_cam1').src = bcc_cam_links[1];
document.getElementById('bcc_cam2').src = bcc_cam_links[2];
document.getElementById('bcc_cam3').src = bcc_cam_links[3];
document.getElementById('bcc_cam4').src = bcc_cam_links[4];
document.getElementById('bcc_cam5').src = bcc_cam_links[5];
document.getElementById('bcc_cam6').src = bcc_cam_links[6];
document.getElementById('bcc_cam7').src = bcc_cam_links[7];

document.getElementById('lcc_cam0').src = lcc_cam_links[0];
document.getElementById('lcc_cam1').src = lcc_cam_links[1];
document.getElementById('lcc_cam2').src = lcc_cam_links[2];
document.getElementById('lcc_cam3').src = lcc_cam_links[3];
document.getElementById('lcc_cam4').src = lcc_cam_links[4];
document.getElementById('lcc_cam5').src = lcc_cam_links[5];
document.getElementById('lcc_cam6').src = lcc_cam_links[6];
document.getElementById('lcc_cam7').src = lcc_cam_links[7];
document.getElementById('lcc_cam8').src = lcc_cam_links[8];
document.getElementById('lcc_cam9').src = lcc_cam_links[9];

document.getElementById('i80_cam0').src = i80_cam_links[0];

// LCC
// WASATCH BLVD @ LITTLE COTTONWOOD
// https://udottraffic.utah.gov/1_devices/aux16607.jpeg
// INTERSECTION OF HWY 209 & HWY 210
// https://udottraffic.utah.gov/1_devices/aux14604.jpeg
// LITTLE COTTONWOOD MP 5.96
// https://udottraffic.utah.gov/1_devices/aux16265.jpeg
// LITTLE COTTONWOOD MP 7.4
// https://udottraffic.utah.gov/1_devices/aux16267.jpeg
// LITTLE COTTONWOOD MP 8.7
// https://udottraffic.utah.gov/1_devices/aux16269.jpeg
// LITTLE COTTONWOOD @ WHITE PINE PARKING MP 9.2
// https://udottraffic.utah.gov/1_devices/aux16270.jpeg
// LITTLE COTTONWOOD MP 9.7
// https://udottraffic.utah.gov/1_devices/aux17227.jpeg
// LITTLE COTTONWOOD @ ALTA BYPASS MP 10.95
// https://udottraffic.utah.gov/1_devices/aux17228.jpeg
// LITTLE COTTONWOOD @ ALTA MP 12.16
// https://udottraffic.utah.gov/1_devices/aux17226.jpeg
// LCC Sign
// http://www.udottraffic.utah.gov/AnimatedGifs/100032.gif

// BCC
// Wasatch Blvd @ Big Cottonwood Canyon
// https://udottraffic.utah.gov/1_devices/aux14605.jpeg
// Big Cottonwood Dogwood MP 4.1
// https://udottraffic.utah.gov/1_devices/aux16212.jpeg
// Big Cottonwood @ S Curves MP 6.38
// https://udottraffic.utah.gov/1_devices/aux16213.jpeg
// Big Cottonwood @ Cardiff Fork MP 10.74
// https://udottraffic.utah.gov/1_devices/aux16215.jpeg
// Big Cottonwood @ Silver Fork MP 12.54
// https://udottraffic.utah.gov/1_devices/aux16216.jpeg
// Big Cottonwood @ Solitude MP 14.5
// https://udottraffic.utah.gov/1_devices/aux18040.jpeg
// Big Cottonwood @ Brighton Circle MP 16
// http://udottraffic.utah.gov/1_devices/SR-190%20MP%2015%2095%20SL.gif
// BCC Sign
// http://www.udottraffic.utah.gov/AnimatedGifs/100033.gif

// Parleys
// I-80 @ Parleys Summit
// http://udottraffic.utah.gov/1_devices/aux166.jpeg
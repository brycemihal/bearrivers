var sID = document.getElementById('plotDischargeRM226').getAttribute('data-siteID');
// console.log(siteID)
// console.log(obs[siteID])
var ObsDT = obs[sID].map(x => x.Datetime);
var ObsDV = obs[sID].map(x => x.Discharge);
var ModDT = mod[sID].map(x => x.Datetime);
var ModDV = mod[sID].map(x => x.Discharge);
...
Plotly.newPlot('plotDischargeRM226', data2);
